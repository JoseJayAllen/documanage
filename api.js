...
const FoldersAPI = {
    ...
    create: async (data) => {
        return await apiCall('/folders/create', 'POST', data);
    },
    update: async (folderId, data) => {
        return await apiCall(`/folders/update/${folderId}`, 'PUT', data);
    },
    delete: async (folderId) => {
        return await apiCall(`/folders/delete/${folderId}`, 'DELETE');
    }
};
...
const FilesAPI = {
    getAll: async (params = {}) => {
        ...
        return await apiCall(`/files${query ? '?' + query : ''}`);
    },
